...
arr = passphrases.each_line.to_a

split_arr = arr.collect{|line| line.split}
# puts split_arr.length # 512 passphrases

filtered_list = split_arr.reject{ |arr| arr.uniq.length != arr.length }
# puts filtered_list.length # 386 unique

sorted = filtered_list.collect do |arr_of_words|
  arr_of_words.collect{ |word| word.split("").sort.join }
end
# puts filtered_list[0]
# puts sorted[0]
no_anagrams = sorted.reject{ |arr| arr.uniq.length != arr.length }
puts no_anagrams.length
