...
arr = input.each_line.to_a

arr.collect! {|s| s.gsub(/[()]/, "")}

no_tops = arr.reject{|s| !s.include?("->")}
split = no_tops.collect{|v| v.gsub(/\s+/, "").split(/[\d+,->]/) }
remove_empties = split.collect do |arr|
  arr.reject{|s| s.length == 0}
end


bottoms = []
tops = []
sort = remove_empties.each do |arr|
  arr.each.with_index do |s, i|
    i == 0 ? bottoms << s : tops << s
  end
end
...
while !answer do
  if !tops.include?(bottoms[counter])
    answer = bottoms[counter]
  end
  counter += 1
end
